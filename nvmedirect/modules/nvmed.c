...
#ifdef CONFIG_X86
        set_memory_uc((unsigned long)nvmeq->cqes, (CQ_SIZE(depth) + PAGE_SIZE - 1) / PAGE_SIZE);
#endif
        if (!nvmeq->cqes)
                goto free_nvmeq;
...
#ifdef CONFIG_X86
        set_memory_uc((unsigned long)nvmeq->sq_cmds, (SQ_SIZE(depth) + PAGE_SIZE - 1) / PAGE_SIZE);
#endif
        if (!nvmeq->sq_cmds)
                goto free_cqdma;
...
static int nvmed_get_device_info(NVMED_NS_ENTRY *ns_entry,
        ...
        // Breaking support for partitions. We don't need it for Open-Channel SSD.
        dev_info.capacity = ns_entry->nr_sects * 512;
        ...
        /*
          The DFC card supports 64 queues. This is too much for our current
          implementation in userspace.
          This workaround will be changed back to the queue count in the futur
          and the limit in userspace will be bound by core count instead.
        */
        dev_info.nr_queues = 8; // dev->queue_count;
...
static void nvmed_init_device(struct nvme_dev *dev, NVMED_DEV_ENTRY *dev_entry,
                              struct nvme_ns *ns, char *sysfsPath,
                              sector_t start_sect, sector_t nr_sects,
                              int partno)
{
        ...
        ns_entry->partno = partno;
        ns_entry->start_sect = start_sect;
        ns_entry->nr_sects = nr_sects;

        if (partno == 0)
                sprintf(dev_name, "nvme%dn%u", DEV_TO_INSTANCE(dev), ns->ns_id);
        else
                sprintf(dev_name, "nvme%dn%up%u", DEV_TO_INSTANCE(dev), ns->ns_id, partno);
        ...
        while ((pdev = pci_get_class(PCI_CLASS_NVME, pdev))) {
                ...
                list_for_each_entry(ns, &DEV_TO_NS_LIST(dev), list) {
                        if (ns->ndev) {
                                ndev = ns->ndev;
                                geo = &ndev->geo;
                                pr_info("NVMeDirect:  - nr_chnls: %d\n", geo->nr_chnls);
                                pr_info("NVMeDirect:  - nr_luns: %d\n", geo->nr_luns);
                                pr_info("NVMeDirect:  - luns_per_chnl: %d\n", geo->luns_per_chnl);
                                pr_info("NVMeDirect:  - nr_planes: %d\n", geo->nr_planes);
                                pr_info("NVMeDirect:  - sec_per_pg: %d\n", geo->sec_per_pg);
                                pr_info("NVMeDirect:  - pgs_per_blk: %d\n", geo->pgs_per_blk);
                                pr_info("NVMeDirect:  - blks_per_lun: %d\n", geo->blks_per_lun);
                                pr_info("NVMeDirect:  - fpg_size: %d\n", geo->fpg_size);
                                pr_info("NVMeDirect:  - pfpg_size: %d\n", geo->pfpg_size);
                                pr_info("NVMeDirect:  - sec_size: %d\n", geo->sec_size);
                                pr_info("NVMeDirect:  - oob_size %d\n", geo->oob_size);
                                pr_info("NVMeDirect:  - mccap %d\n", geo->mccap);
                                partno = 0;
                                start_sect = 0; /* For now this is hard coded for the DFC */
                                nr_sects = (sector_t)(geo->nr_chnls) *
                                        geo->luns_per_chnl *
                                        geo->nr_planes *
                                        geo->blks_per_lun *
                                        geo->pgs_per_blk *
                                        geo->fpg_size / 512UL;
                                pr_info("NVMeDirect:  - nr_sects %lu\n", nr_sects);
                                pr_info("NVMeDirect:  - nr_sects %lx\n", nr_sects);
                                nvmed_init_device(dev, dev_entry, ns, sysfsPath,
                                                  start_sect, nr_sects, partno);
                                continue;
                        }

                        disk_part_iter_init(&piter, ns->disk, DISK_PITER_INCL_EMPTY_PART0);
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Frey Alfredsson <frea@itu.dk>");
MODULE_AUTHOR("Hyeong-Jun Kim <hjkim@csl.skku.edu>");
MODULE_DESCRIPTION("Modified NVMeDirect Module for OpenChannelSSD");
MODULE_VERSION("0.9");
